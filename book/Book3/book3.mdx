...
Everything traces to ten axioms and two constants: the golden ratio φ ≈ 1.618 (from self-reference) and the circle constant π ≈ 3.14159 (from self-referential geometry). No adjustable parameters are used (two dimensionful anchors set the scale). No parameters are adjusted. The framework either fits or it doesn't — and so far, with fifty predictions across seven domains, it fits.
...
### The worst prediction in physics — and why the framework doesn't have it
...
The lattice spacing *a* appears to be a free input — but as shown below, it is determined by the Fibonacci sequence. The coupling unification analysis (above) constrains it to about 85 Planck lengths, but does not derive it from axioms. If *a* could be derived, Newton's gravitational constant *G* would follow, and the framework would have zero adjustable parameters.
...
*, and the screening exponent an instance of the same fixed-point calculus that produces the fine-structure constant. See the update section below.)*
