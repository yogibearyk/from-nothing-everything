...
*Claim status: Structural observation (consequence of axioms + standard physics)*
...
**Why zero adjustable parameters.** A ground state has no knobs to turn. It is what it is — the unique lowest-energy configuration of a specific system. You cannot adjust it without changing the system (the axioms). Change the axioms, and you get a different ground state with different constants. Keep the axioms, and the constants are fixed.
...
**Why the cosmological constant is not fine-tuned.** The standard CC problem asks: why does the sum of all quantum fluctuations cancel to 122 decimal places? Ground state selection reframes the question: the observable vacuum energy is the ground state energy density as seen by a stage-6 observer through the Drishti screening. The screening depth (D⁴²⁴ ≈ 10⁻¹²²) is a property of stage 6, not a cancellation between large numbers. At stage 5, the screening depth would be 10⁻¹⁹. At stage 7, it would be 10⁻²⁴⁵³. The value 10⁻¹²² is not fine-tuned — it is the unique screening depth at our stage.

**Why some predictions are more precise than others.** The formulas in this book — D⁴²⁴ for the CC, the Koide formula for lepton masses, the self-referential deficit for α — are not the final answers. They are **approximations to the exact ground state properties**, derived from the lattice's topology without full dynamical computation. Some approximations are excellent (α to six significant figures); others are rough (Λ to within a factor of 6). The exact values require computing the ground state itself — a defined problem (3D Monte Carlo simulation, renormalization group flow) that would yield all fifty constants simultaneously from one computation.
...
Some frontiers have closed since this chapter was first written — absolute masses, individual lepton masses, and the CMB parameters are proposed (conditional on the post-comparison H₀ correction; the acoustic-scale tension documented in Chapter 30 remains unresolved) (Chapters 17, 29, and 30). What remains genuinely open:
