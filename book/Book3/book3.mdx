...
So the crack is real, and it is recorded. But it is the significant unresolved tension by a millimeter, not of a wall falling. The framework's CMB predictions are not fundamentally broken — they are an unresolved tension that requires either independent confirmation of the H₀ correction or a structural revision of the CMB derivation.
...
The framework proposes five cosmological parameters (conditional derivations) plus the optical depth τ (external Planck input — see Chapter 30) from its axioms (plus one number borrowed from astrophysics — the optical depth τ, which measures how much the universe's later history reionized the primordial gas). The five derived values, plus one external input, with their sources:
...
- **How much dark radiation:** ΔN_eff ≈ 0.02, from the dark sector's temperature ratio (post-comparison diagnostic — the thermal history mechanism was invalidated; see Chapter 30).

These five derived numbers, plus the external optical depth τ, were fed into **CAMB** — a standard, publicly available computer program that solves the Boltzmann equations governing how light, matter, and gravity interact in the early universe. CAMB is not ours; it is the same tool used by the Planck collaboration and by hundreds of cosmology groups worldwide. We gave it our numbers; it gave us a curve.

### What happened

At H₀ = 68.2 (pre-correction), the curve matched at 2,500 angular scales — the framework's predicted power spectrum was statistically indistinguishable from the Planck satellite's best-fit spectrum — not just for the temperature ripples (TT), but also for the polarization patterns (EE and TE), which are independent measurements that test different physics. The agreement held at the level of **cosmic variance** — the irreducible statistical noise from the fact that we have only one universe to observe.
