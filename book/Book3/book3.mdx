...
**What is established:** A discrete Fibonacci/Zeckendorf hard-core lattice with exact arithmetic, natural exclusion rules, and constrained state spaces. From this core, the golden ratio φ and the circle constant π emerge with mathematical necessity. The Drishti screening ratio D = φ/π bridges the algebraic and transcendental worlds.
...
Three bridges → three spatial dimensions. Combined with the one time direction from Step 5 → four spacetime dimensions total. This is identified (structural correspondence) with the tower's carry cascade count.
...
| 8 | 21 | 1 | 10 | 11 | Matches M-theory's 11 dimensions |
...
**Why three force-like sectors:** the Fibonacci recurrence — the defining property of the sequence, not a choice among partitions — decomposes the 8-position tower. Each Fibonacci number equals the sum of the two before it: 8 = 5 + 3 (smaller group: **3** positions), then 5 = 3 + 2 (smaller: **2**), then 3 = 2 + 1 (smaller: **1**). The sizes 3, 2, 1 — consecutive Fibonacci numbers — determine the three gauge symmetries: SU(3) for the strong force (3 interchangeable "colors"), SU(2) for the weak force (2 interchangeable states), and U(1) for electromagnetism (1 phase rotation). This is the Standard Model gauge group, identified (structural correspondence) from the tower's arithmetic.

Three things determine the gauge group: the **dimension** (how many bridges — 3, 2, 1 from Fibonacci), the **type** (real or complex — the bilateral M₁/M₂ structure makes each bridge complex, requiring unitary groups SU(N) rather than orthogonal groups SO(N)), and the **classification** (SU(N) is the unique simple compact Lie group acting faithfully on N complex dimensions, a result from the Killing-Cartan classification of Lie algebras). Three complex bridges → SU(3). Two → SU(2). One → U(1). The framework constrains the gauge group more tightly than standard lattice QCD, which imposes SU(3) by hand. However, explicitly constructing the 8 generators of SU(3) as lattice operators on the Fibonacci lattice — writing the Wilson action with SU(3) link variables and verifying gauge invariance — has not been performed. The tools exist (they are the same tools used in every lattice QCD simulation). The construction is defined future work.

At stage 7 (the dark sector), the same decomposition produces a fourth symmetry — **SU(5)** — the grand unified group that physicists have sought for fifty years. The framework predicts it exists at the next stage, screened from our view, and that the **proton is exactly stable** (no proton decay, ever — a distinguishing prediction).
...
These shadows are **nested like Russian dolls**: our 4-dimensional shadow fits inside the dark sector's 7-dimensional shadow, which fits inside the 11-dimensional shadow. Each layer contains all inner layers plus new directions invisible from within.
...
**The precision frontier** — the electroweak scale to 0.66 parts per million (derived from three orthogonal bridges × the gravitational self-coupling of the spatially uniform mode), and the quark-lepton mass splitting to 2.3% with a unique mass reversal prediction confirmed by observation.
...
**Method B (bottom-up, microscopic):** each lattice cell on the horizon surface has area a² = F(11)² Planck areas — 7,921 "pixels," each a single Planck area. The horizon is a 2D surface spanned by 2 of the tower's 3 bridges. Each bridge has a bilateral structure (forward and backward). Each physical configuration therefore appears in 4 orientational copies (2 bridges × 2 bilateral). Counting each configuration once: F(11)²/4 = 1980 independent degrees of freedom per cell. The total entropy: S = (A/a²) × a²/(4l_P²) = A/(4G).
...
**Theorem (Carry Cascade Count).** The carry cascade produces ⌈d_S/2⌉ active positions at stage S. At stage 6: ⌈8/2⌉ = 4. **Structural correspondence:** The framework identifies these 4 active positions with the 4 dimensions of spacetime — 3 spatial (from the 3 bridges) and 1 temporal (from the single negative product M₁ × M₂ = −1).
...
At **stage 8**: 21 positions → 11 dimensions → **10 space + 1 time.** This matches the dimensionality of **M-theory**, the leading candidate for a unified theory of all forces including gravity, developed by string theorists in the 1990s. Whether this numerical match reflects a deep connection or a coincidence is an open question — the framework notes it honestly without claiming to have derived M-theory.

At **stage 7** (the dark sector): 13 positions → 7 dimensions → **6 space + 1 time.** The dark sector has three extra spatial dimensions beyond our three — internal directions that dark matter particles may move in, but that are screened from our observation. These extra dimensions are not curled up too small to see (as some theories propose). They are screened — present at every lattice site but coupled to us too weakly to detect, except through gravity.
...
**Level 1:** 8 = 5 + 3. The tower splits into a group of 5 and a group of 3, connected by a bridge. The smaller group has **3 positions**.

**Level 2:** The larger group (5) splits: 5 = 3 + 2. Connected by a second bridge. The smaller group has **2 positions**.

**Level 3:** That larger group (3) splits again: 3 = 2 + 1. Connected by a third bridge. The smaller group has **1 position**.

Three bridges. Three smaller groups. Sizes: **3, 2, 1** — consecutive Fibonacci numbers F(4), F(3), F(2).
...
**A bonus prediction: grand unification at stage 7.** At stage 7 (the dark sector, 13 positions), the first split is 13 = 8 + 5. The smaller group has 5 positions → **SU(5)**. This is the **Georgi-Glashow grand unified group** — the simplest group that contains the entire Standard Model. Physicists have searched for evidence of SU(5) unification for fifty years. The framework says it exists at stage 7, screened from our view by the Drishti bound. Grand unification is real — we just can't see it directly because it's screened.
...
The resolution comes from the SU(5) at stage 7. In mathematics, SU(5) contains SU(3) × SU(2) × U(1) as its unique maximal subgroup of this form. "Unique" means there is only ONE way to fit the three Standard Model forces inside SU(5). That fit determines a single, specific U(1) — not three.

The three U(1) factors from the stage-6 decomposition are not independent. They are three views of the SAME U(1), forced to coincide by the SU(5) structure at stage 7. The bonus prediction (SU(5)) turned out to be essential for closing this gap — a sign of internal consistency.
...
The reason is the **Stage Isolation Theorem** (Chapter 22). The SU(5) symmetry lives at stage 7 — the dark sector. The particles that would mediate proton decay (called **X and Y bosons**) are stage-7 particles. They interact with dark-sector matter, not with our matter. Stage Isolation forbids gauge interactions from crossing the stage boundary — only gravity crosses it.

Think of it as two copies of the grand unified force: one in our visible world (SU(3) × SU(2) × U(1)) and one in the dark world (SU(5) containing dark SU(3) × dark SU(2) × dark U(1)). The dark copy can cause dark proton decay. But it cannot reach our protons. The stage boundary is an impassable wall for everything except gravity.
...
With the hypercharges forced by the SU(5) embedding (1/6, −2/3, 1/3, −1/2, and 1 for the five types of fermions), all six sums are exactly zero. Every single one. The framework's fermion content is mathematically self-consistent.
...
The SU(5) decomposition assigns each tower position to a specific type of particle. The 5̄ + 10 representation of SU(5) (the unique anomaly-free combination with 15 particles per generation) maps as follows:
...
**Theorem NMS.0 (right-handed neutrino from axioms).** The stage-6/7 boundary mode is a complete SM gauge singlet = ν_R. *Proof:* Stage Isolation Theorem (Appendix A) → position ≥ 8 is SU(3)_c singlet (bridges at 3,5,7 are below), SU(2)_L singlet (binary encoding is within stage 6), U(1)_Y = 0 (triangle node). Axiom 5 (persistence) → the boundary mode exists as a dynamical fermion. *Majorana nature:* The full-tower mode (all 8 positions occupied) is self-conjugate under tower inversion p ↔ (7−p); a self-conjugate gauge singlet IS Majorana. *Assumption:* The relevant boundary mode is the full-tower configuration; partial occupancy would not guarantee self-conjugacy. ■
...
The thirteen-petal flower at stage seven has a richer internal structure than our eight-petal stage six. The dimensionality formula gives ⌈13/2⌉ = 7 — six spatial dimensions plus time. From our perspective, those extra dimensions are screened away. We see the dark sector's matter as featureless dark mass in our three dimensions of space. But internally, the dark flower has six directions to unfold in, not three. It is a richer universe than ours, hidden behind one layer of golden-ratio glass.
...
The graph's topology is what determines the physics. Three bridges produce three spatial dimensions. The adjacency pattern produces the hard-core constraint (Pauli exclusion). The ordering produces the carry cascade (forces). Everything the framework derives comes from the topology of this graph — not from any spatial arrangement of the nodes within the site.
...
Here is something remarkable. Stage eight — twenty-one petals — has ⌈21/2⌉ = 11 internal dimensions. Eleven dimensions is exactly the dimensionality of M-theory, the proposed unification of all string theories. The framework does not import this number. It produces it as the natural dimensionality of the third ring of the flower. Whether this connection to string theory is deep or coincidental is an open question — but the number 11 is not put in by hand.
...
**Do magnetic monopoles exist?** Grand unified theories predict particles with a single magnetic pole (north without south) — **magnetic monopoles**. These would be created during the GUT phase transition, when the unified force splits into the three separate forces. The framework predicts no magnetic monopoles exist. The reason: Stage Isolation means there is no GUT phase transition in the visible sector. The SU(5) unification lives in the dark sector (stage 7), separated from the visible sector by a screening boundary that no topological defect can cross. No phase transition → no topological defects → no monopoles. This is a testable prediction: if a magnetic monopole is ever detected, the framework is falsified.
...
The answer is yes, automatically. The framework proposes the Standard Model Lagrangian as its candidate continuum action: the gauge group (SU(3)×SU(2)×U(1), from Fibonacci), the matter content (5̄+10 per generation, from SU(5)), all coupling constants (from D = φ/π), all particle masses (from Koide and screening), and all mixing angles (CKM, PMNS). The Lagrangian IS the dynamics — every scattering amplitude, decay rate, and cross-section follows from it through standard quantum field theory. Computing a specific process from the derived Lagrangian is a textbook exercise applied to derived inputs.
...
**The second-order coefficient (M12).** |k| = d × k_TOV = 3 × 3/4
