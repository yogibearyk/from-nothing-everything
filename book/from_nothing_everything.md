...
In Chapter 10, we saw the tower growing through Fibonacci stages — each stage deeper than the last. But we skipped over something that happens at every stage, something the diagram below makes visible. Look at the left column first: at each stage, every position in the tower fills up. Then look at the right column: the tower rearranges, and an alternating pattern appears. This chapter explains why.

![The tower grows and the cascade fires at each stage](images/carry_cascade_all_stages.svg)

**Why does the tower fill up?** Because growth means something arrives. Each new position is generated by the two before it (Chapter 3). A position that arrives empty — that generates nothing, contributes nothing — would be a growth step that didn’t grow. That contradicts the growth rule. So every position arrives active: filled, on, set to 1.

**Why does it rearrange?** Because of the carry rule from Chapter 5. The counting system says: two adjacent 1s cannot coexist — they must combine into one 1 at the next level up (11 → 100). At stage 3, the tower has two positions, both on: 11. That’s one adjacent pair. The pair combines, and the tower rearranges. At stage 4, three positions: 111. At stage 5, five: 11111. At stage 6, eight: 11111111. The more positions, the more adjacent pairs, the more dramatic the rearrangement.

**How does the rearrangement work?** Start at the bottom of the tower and work up. Take each pair of adjacent 1s and combine them: the two become one at the next position. At stage 6: the first pair (positions 0 and 1) combines into position 2. The next pair (positions 2 and 3) combines into position 4. Then 4 and 5 combine into 6. Then 6 and 7 combine into 8. Four pairs, four combinations. The result: active positions at 2, 4, 6, and 8 — every other spot — with gaps at 1, 3, 5, and 7. The overcrowded all-1s tower has resolved into a clean alternating pattern.

The number of active positions that remain tells us something remarkable. At stage 3: 1 active position. At stage 4: 2. At stage 5: 3. At stage 6: **4** — exactly the number of spacetime dimensions we observe. Whether this is coincidence or consequence is what Chapter 12 explores.

One final detail that matters for the larger story. The last combination at each stage produces a carry that *exits* the tower — it overflows past the top. That overflow doesn’t disappear. It becomes the seed of the next stage. The rearrangement at stage 5 overflows into stage 6. The rearrangement at stage 6 overflows into stage 7. Each stage resolves its own overcrowding, and the overflow starts the next epoch. Growth creates tension; the carry rule resolves it; the resolution’s overflow restarts growth. The process never stops.

This is not special to stage 6. It happens at every Fibonacci stage, from the very first carry at stage 3 onward. What physicists call the “Higgs mechanism” — the event that gives particles their mass and breaks the symmetry of the forces — is, in the framework’s reading, the stage-6 instance of this universal process. It is not a unique moment in the history of the universe. It is what growth always does when it outgrows its own rules.
