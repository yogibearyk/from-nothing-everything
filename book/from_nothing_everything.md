...
But the tower does not stop at stage 6. The axioms contain no stop clause. Stage 7 produces a dark sector — invisible matter detectable only through gravity. Stage 8 produces 11 spacetime dimensions, matching the dimensionality required by M-theory. The tower continues without limit, each stage more screened than the last, the total converging to a finite value: 1/π. The infinite tower is the journey from Ω back to Ω — infinite in stages but finite in content. And the “worst prediction in physics” — the 10¹²² mismatch between the predicted and observed energy of empty space — dissolves into 212 ordinary screenings of the same Drishti bound, each dimming by a quarter, compounding to give the observed ratio exactly.
