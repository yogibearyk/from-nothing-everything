...
**The cosmological constant problem** — dissolved (the 10¹²⁰ discrepancy doesn't arise because the lattice provides a natural cutoff, and the value of Λ is derived directly to 0.03σ).

**The complete dynamical action** — the single mathematical expression from which all physical processes follow. The action has four parts: gravity (from Lovelock's theorem), the three forces (from the Wilson gauge action with derived couplings), matter (from the fermion action with the hard-core constraint), and the Higgs field (with the derived vacuum expectation value). Every coefficient is derived from the axioms. Scattering amplitudes, decay rates, and cross-sections all follow from this action through standard quantum field theory — the same equations particle physicists use, applied to derived inputs instead of measured ones.
...
**Dynamics.** The framework derives both the constants of physics AND the rules of motion. The complete dynamical action S = S_gravity + S_forces + S_matter + S_Higgs (Chapter 31) contains every coefficient derived from the axioms. The Einstein-Hilbert action governs gravity (from Lovelock). The Wilson gauge action governs the three forces (with derived couplings). The fermion action governs matter (with derived masses and the hard-core constraint). The Higgs action governs mass generation (with the derived VEV). Three specific processes — the Z boson decay width, the neutron lifetime, and the Higgs decay to bottom quarks — have been computed from the derived action and match measurements to 0.7%, 4%, and ~5% respectively at leading order.
