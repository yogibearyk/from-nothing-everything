...
**Quantum mechanics itself.** The framework uses quantum mechanics — superposition, probability amplitudes, the uncertainty principle, entanglement — as its mathematical language. But it does not derive WHY the universe is quantum mechanical. The **Born rule** (the rule that says probabilities are the square of amplitudes — the reason quantum mechanics makes statistical predictions rather than deterministic ones) is inherited from standard physics, not produced by the axioms. The **measurement problem** (what happens physically when a quantum system is observed and appears to "collapse" into a single outcome) is not addressed. The framework derives what happens INSIDE quantum mechanics (the specific constants). It does not derive why quantum mechanics is the right framework in the first place.

Whether the lattice's discrete structure — where each position is either on or off, and the hard-core constraint forces specific correlations — might naturally produce quantum behavior is an open question. Some researchers have explored whether quantum mechanics emerges from underlying discrete dynamics (this is called the "emergent quantum mechanics" program). The framework's lattice is a natural candidate for such emergence, but this has not been shown.
