...
Two structural results have been strengthened through exact lattice computation: the gravitational coefficient |k| = 9/4 (structural hypothesis: the three-dimensional emergent arena × the standard interior coefficient, with an honest open question about whether that multiplication double-counts — see Chapter 30) and the quark-lepton mass splitting coefficient (from the identity φ² = φ + 1). Both are documented with their claim status in the relevant chapters.
...
**Theorem (Universal Carry Cascade).** At every stage S ≥ 3, the all-1s tower of depth d_S resolves into the alternating pattern with ⌈d_S/2⌉ active positions, plus a boundary carry at φ^(d_S). The value is preserved exactly through the cascade. (Proof status: verified stage-by-stage to stage 12 at 200-digit precision; the general all-depths proof — via the value identity V(d) = F(d+3) − 2, i.e. the all-1s tower of depth d always sums to two less than a Fibonacci number — is defined future work.)
...
With the standard hypercharges (1/6, −2/3, 1/3, −1/2, and 1 for the five types of fermions), all six sums are exactly zero. Every single one. (Earlier editions derived these hypercharges from an SU(5) embedding; with SU(5) falsified, the framework's route is the reverse — and stronger — one: demand that all six anomaly sums vanish for the derived particle content, and these hypercharge values are essentially the only solution. Anomaly freedom itself fixes the charges. Writing out that uniqueness argument in full is flagged work.) The framework's fermion content is mathematically self-consistent.
...
Each tower position hosts a specific type of particle. The count — 15 fermions per generation, arranged as color triplets, weak doublets, and singlets — is fixed group-by-group by the construction blocks (3, 2, 1) together with anomaly freedom. (Earlier editions organized this as the 5̄ + 10 of SU(5); with SU(5) falsified, the same 15-per-generation content stands on the Standard-Model groups directly, and the detailed position-by-position assignment is flagged for re-derivation.) The mapping:
...
The connection: the frontier mode's gravitational well affects the screening through all three spatial directions of the emergent arena simultaneously. In the smooth large-scale limit the arena is isotropic — the same in every direction: the microscopic links, though individually lopsided, average out, just as a square grid looks perfectly round from far enough away. Each of the three directions contributes its own curvature correction of 3/4 from the interior of the well. Three directions, each contributing 3/4: the total coefficient is **9/4 = 2.25**, matching the data to 0.06σ. (Caveat, recorded honestly: whether the interior coefficient 3/4 is genuinely a per-direction quantity — so that multiplying by three is legitimate — or already contains the three-dimensionality of general relativity, has not been established. This double-counting question is the open core of the structural hypothesis.)
...
- **The second-order coefficient (structural hypothesis):** |k| = d × k_TOV = 3 × 3/4 = 9/4 = 2.25 (0.06σ from target). Here d = 3 is the emergent arena's dimensionality (isotropic in the continuum limit — not "three orthogonal bridges"; the microscopic links are asymmetric and only average to isotropy at large scales), and 3/4 is the standard GR interior (Tolman) coefficient. Open sub-question: whether 3/4 is per-direction, legitimizing the ×3 (Chapter 30 caveat). v = 246.2195 GeV (0.66 ppm). Structural hypothesis tier.
...
|k| = [F(4)/F(3)]² = (3/2)² = **9/4 = 2.25** — an alternative decomposition of the same number. Two different factorizations (3 × 3/4 and (3/2)²) both landing on 9/4 is a caution, not a strength: a fitted coefficient admits many after-the-fact factorizations. One mechanism should ultimately be selected; until then both are recorded as candidates.
...
The answer is yes, automatically. The framework proposes the Standard Model Lagrangian as its candidate continuum action: the gauge group (SU(3)×SU(2)×U(1), from Fibonacci), the matter content (15 fermions per generation, from the construction blocks plus anomaly freedom; the detailed assignment is flagged for re-derivation), all coupling constants (from D = φ/π), all particle masses (from Koide and screening), and all mixing angles (CKM, PMNS). The Lagrangian IS the dynamics — every scattering amplitude, decay rate, and cross-section follows from it through standard quantum field theory. Computing a specific process from the derived Lagrangian is a textbook exercise applied to derived inputs.
...
**The second-order coefficient (M12).** |k| = d × k_TOV = 3 × 3/4 — with d = 3 the emergent arena's dimensionality (isotropic at large scales; not bridge geometry), and with the open question of whether the interior coefficient is per-direction (Chapter 30 caveat) —
