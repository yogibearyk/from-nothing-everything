...
The tower has 3 link positions (3, 5, 7 — the vacated carry sources of the cascade). On this route they are structurally equivalent after the cascade (the screening D applies per STAGE, not per individual position within a stage, so all link positions at the same stage carry the same hopping amplitude). The filling is incommensurate (from the golden ratio's irrationality), ruling out the Mott insulator. The SU(3) GLOBAL flavor symmetry generators exist (Schwinger construction on 3 bridges, independently verified).
