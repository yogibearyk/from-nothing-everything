...
Three things determine the gauge group: the **dimension** (the sizes 3, 2, 1 of the construction blocks — the tower's growth record), the **type** (real or complex — the bilateral M₁/M₂ structure makes each block's amplitudes complex, requiring unitary groups SU(N) rather than orthogonal groups SO(N)), and the **classification** (SU(N) is the unique simple compact Lie group acting faithfully on N complex dimensions — the Killing-Cartan classification of Lie algebras). And there is a ceiling: because the carry rule reaches back exactly two steps, at most **three** consecutive positions can all be directly linked to one another — three is the largest possible clique. So SU(3) is the largest gauge symmetry the framework can ever produce, at any stage. (In plain terms: the same "reach of two" that gives three spatial dimensions also caps the strongest force at three colors.) The framework constrains the gauge group more tightly than standard lattice QCD, which imposes SU(3) by hand. However, explicitly constructing the 8 generators of SU(3) as lattice operators — writing the Wilson action with SU(3) link variables and verifying gauge invariance — has not been performed. The tools exist (the same tools used in every lattice QCD simulation). The construction is defined future work — and it now also underwrites the Stage Isolation statement (Chapter 22), raising its priority.
...
- **The second-order coefficient (structural hypothesis):** |k| = d × k_TOV = 3 × 3/4 = 9/4 = 2.25 (0.06σ from target). Here the honest form is |k| = (d/2)² — the square of the center-to-edge depth ratio of a uniform gravitational well in d dimensions (proven: the ratio is d/2 for any d), entering twice because the term is second order. The older "d × k_TOV" reading is the same formula (k_TOV = 3/4 is itself (1/2)(d/2), a scalar already containing d); the once-open per-direction question is resolved — dissolved into the unified mechanism. Conditionals labeled: uniform source profile; second-order entry (frontier action). v = 246.2195 GeV (0.66 ppm). Structural hypothesis tier.
