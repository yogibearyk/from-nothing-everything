...
In the Fibonacci decomposition 8 = 5 + 3, the SU(5) grand unified group decomposes as 5 = 3 + 2, giving SU(3) from the 3-register and SU(2) from the 2-register. The U(1) hypercharge emerges from the remaining 1-register. The U(1) generator's trace normalization within SU(5) introduces the standard GUT factor 5/3 — which in Maya math is F(5)/F(4), the ratio of consecutive Fibonacci numbers at the decomposition level.

The bare U(1) coupling: g₁² = (5/3) × D² = (F(5)/F(4)) × (φ/π)². This is the SAME normalization factor used in every GUT textbook. In the framework, it comes from the Fibonacci decomposition that produces SU(5): the ratio F(5)/F(4) = 5/3 is a structural fact about the decomposition, not an imported constant.
...
| β_{U(1)} | (5/3)D² = F(5)/F(4) × D² | GUT normalization from SU(5) Fibonacci decomposition |
| m | (√(1+4D²)−1)/2 | 0.2178 |
