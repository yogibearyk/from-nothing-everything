...
Of the thirty-two predictions that can currently be compared to measurements, **all thirty-two match within two standard deviations.**
...
### Why exactly one U(1) — resolved by stage 7
...
A doubler oscillates with alternating signs at adjacent lattice sites: +, −, +, −... This is exactly the pattern the hard-core constraint forbids — two adjacent sites both being active (non-zero). The Zeckendorf constraint operates along each of the 4 spacetime directions (3 bridges + time) independently. Any mode with any momentum component at the lattice cutoff (π/a) is eliminated. The only surviving mode is the physical fermion at zero momentum — exactly one copy, with no doublers.
