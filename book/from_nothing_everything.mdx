...
The microscopic black hole state-counting is now complete: each lattice cell on the horizon surface contains F(11)² = 7921 Planck-scale patches, reduced by the bilateral structure (÷4) to 1980 independent degrees of freedom — matching the Wald formula S = A/(4G) exactly. Two independent methods (the Wald theorem from the action, and the Planck-patch counting from the lattice) give the same entropy. Everything else — every coupling constant, every mass ratio, every mixing angle, the gravitational sector, the cosmological parameters, the dark sector, the spatial structure, the equations of quantum mechanics and general relativity, and the measurement problem's structural context — is closed.
...
In the framework, the imbalance comes from the asymmetry between the observer (M1 = φ) and the observed (M2 = −1/φ). These have different magnitudes — the observer is larger. This asymmetry, screened through the full bilateral tower (16 bilateral positions = 8 positions × 2 directions), gives the **baryon-to-photon ratio**: η = φ × D̃¹⁶ = φ × (φ/(2π))¹⁶ ≈ 6.05 × 10⁻¹⁰. The measured value is (6.12 ± 0.04) × 10⁻¹⁰ — a match to 1.1%, from zero parameters.
