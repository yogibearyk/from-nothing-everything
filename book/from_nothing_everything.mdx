...
The self-referential closure operates at two levels. First: the same Fibonacci decomposition (8 = 5 + 3) that creates the strongly correlated bridge structure (3 bridges with PXP interactions) ALSO provides the spatial dimensionality (3D) where Fermi liquid behavior is generic. Second: the same golden ratio (φ) that creates the hard-core constraint ALSO makes the filling irrational, preventing the Mott insulator phase. Both the spatial structure and the density structure conspire — through the same mathematical origin — to ensure the low-energy physics is a free Fermi liquid.
