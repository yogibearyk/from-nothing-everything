...
**Step 3.** The something has no features — no preferred direction, no preferred point. The closed path inherits this featurelessness. The unique closed curve with no distinguishable point or direction is the circle.
...
**Caveats.** (a) The 32 predictions share φ and ⊙ — a rigorous Bayesian model comparison accounting for this correlation remains to be done. (b) The strongest refutation of coincidence will come from the unmeasured predictions (r, Σm_ν, normal ordering), which cannot be reverse-engineered.
