...
**The Standard Model's structure.** The framework derives the numbers in the Standard Model's equations (coupling constants, masses, mixing angles). It does not derive the FORM of the equations — why the forces are described by the specific mathematical structure called **SU(3) × SU(2) × U(1)** (a notation that describes the symmetry groups of the three non-gravitational forces), why there are exactly the particles there are, or why the interactions take the specific form they do. The bridge structure at stage 6 (three bridges, three gauge groups) is suggestive — it hints that the gauge group structure follows from the tower's topology — but this hint has not been turned into a derivation.
