...
The result is a new interpretation that might be called **lattice realism**: the lattice is real, definite, and non-local. Quantum mechanics — with all its superpositions, uncertainties, and probabilities — is the emergent description for observers who cannot see the full lattice. It is simpler than pilot wave theory (which needs both particles AND a wave function as separate entities), more specific than many-worlds (which doesn't explain why we experience one branch), and more explanatory than Copenhagen (which declares "don't ask" about what happens during measurement).
